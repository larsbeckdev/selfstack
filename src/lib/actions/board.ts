...
// ─── Layout Actions (react-grid-layout) ──────────────────────────────────────
...
/** Persist outer-grid (category) layout for a board. */
export async function saveCategoryLayout(
  boardId: string,
  items: z.infer<typeof layoutItemSchema>[],
) {
  await requireBoardAccess(boardId, "editor");
  const parsed = z.array(layoutItemSchema).parse(items);
...
/** Persist inner-grid (tile + group header) layout for a category. */
export async function saveCategoryInnerLayout(
  categoryId: string,
  tiles: z.infer<typeof layoutItemSchema>[],
  groups: z.infer<typeof layoutItemSchema>[],
) {
  const category = await db.category.findUnique({
    where: { id: categoryId },
    select: { boardId: true },
  ...
  const parsedTiles = z.array(layoutItemSchema).parse(tiles);
  const parsedGroups = z.array(layoutItemSchema).parse(groups);

  await Promise.all([
    ...parsedTiles.map((item) =>
      db.tile.update({
        where: { id: item.i },
        data: {
          layoutX: item.x,
...
/** Move a tile to a new group (cross-category drag). */
export async function moveTileToGroupWithLayout(
  tileId: string,
  newGroupId: string,
  ...
  await revalidateBoard(newGroup.category.boardId);
  if (newGroup.category.boardId !== tile.group.category.boardId) {
    await revalidateBoard(tile.group.category.boardId);
  }
}
